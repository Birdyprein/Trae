      ...
      <table className="w-full text-sm whitespace-nowrap">
        <thead>
          <tr className="border-b border-surface-divider">
            ...
            {funds.map((fund, idx) => (
              <th key={fund.id ?? fund.code ?? idx} className={`px-4 py-3 min-w-[140px] ${idx > 0 ? 'border-l border-surface-divider' : ''}`}>
              ...
              <Fragment key={`row-${rIdx}-${row.label}`}>
                ...
                <tr className={`glass-table-row ${rIdx % 2 === 0 ? 'bg-white/[0.02]' : ''}`}>
                  ...
                  {funds.map((fund, idx) => {
                    ...
                    return (
                      <td key={fund.id ?? fund.code ?? idx} className={`px-4 py-2.5 text-center ${idx > 0 ? 'border-l border-surface-divider' : ''}`}>
